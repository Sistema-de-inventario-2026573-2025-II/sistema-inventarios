...
8.  Under **Connection parameters**, note down these values:
    ...
    *   **Password** (`POSTGRES_PASSWORD`) - The one you set in step 3.
    ...
    > Supabase's "Direct Connection" is IPv6 only. Render (Free Tier) often requires IPv4.
    > To fix this, use the **Connection Pooling** settings in Supabase:
    > 1. Click the **"Connection Pooling"** toggle (or go to Settings -> Database -> Connection Pooling).
    > 2. Use the **Host** provided there (e.g., `aws-0-us-east-1.pooler.supabase.com`).
    > 3. Use **Port** `6543` (Transaction Mode) or `5432` (Session Mode).
    > 4. Use these values for `POSTGRES_SERVER` and `POSTGRES_PORT` in Render.
...
6.  **Environment Variables:** You will be asked to fill in the values for the database. Use the values from Supabase:
    *   `POSTGRES_SERVER`: Copy from Supabase (Pooler URL).
    *   `POSTGRES_PORT`: 6543 (or 5432).
